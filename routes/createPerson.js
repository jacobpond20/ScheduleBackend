...
router.post('/', async (req, res) => {
    ...
    for (s of await Shift.find({mon: true})){
        if(req.body.mon_start <= s.start && req.body.mon_end >= s.end){
            mon.push(s);
        };
    };
    for (s of await Shift.find({tue: true})){
        if(req.body.tue_start <= s.start && req.body.tue_end >= s.end){
            tue.push(s);
        };
    };
    for (s of await Shift.find({wed: true})){
        if(req.body.wed_start <= s.start && req.body.wed_end >= s.end){
            wed.push(s);
        };
    };
    for (s of await Shift.find({thu: true})){
        if(req.body.thu_start <= s.start && req.body.thu_end >= s.end){
            thu.push(s);
        };
    };
    for (s of await Shift.find({fri: true})){
        if(req.body.fri_start <= s.start && req.body.fri_end >= s.end){
            fri.push(s);
        };
    };
    for (s of await Shift.find({sat: true})){
        if(req.body.sat_start <= s.start && req.body.sat_end >= s.end){
            sat.push(s);
        };
    };
    for (s of await Shift.find({sun: true})){
        if(req.body.sun_start <= s.start && req.body.sun_end >= s.end){
            sun.push(s);
        };
    };
    let ava = new Avail({
